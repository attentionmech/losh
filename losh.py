...
class TransformersBackend(AbstractBackend):
    ...
    def load_dataset(self, dataset_name: str, dataset_config: str = "wikitext-2-raw-v1", split: str = "train[:1%]"):
    ...
    def generate_text(self, prompt: str, max_length: int = 100, **kwargs) -> str:
        ...
        text_generator = pipeline("text-generation", model=self.model, tokenizer=self.tokenizer, device=0 if self.device == "cuda" else -1)
...
class MLXBackend(AbstractBackend):
    ...
    def preprocess_dataset(self, max_length: int = 128):
        ...
        def tokenize_function(examples):
            ...
            for text in examples["text"]:
                ...
                if len(tokens) > max_length:
                    ...
                    tokens = tokens + [self.tokenizer.eos_token_id or 0] * (max_length - len(tokens))
        ...
        self.processed_dataset = {
            "input_ids": mx.array([sample["input_ids"] for sample in tokenized_dataset], dtype=mx.int32)
        }

    def finetune(self, output_dir: str = "./mlx_results", num_train_epochs: int = 1):
        if self.processed_dataset is None:
            raise ValueError("No preprocessed dataset available. Call preprocess_dataset first.")
